...
SUBSCRIBES TO:
    ...
    /camera/rgb/image_raw: Source image topic
...
class BlobDetector:
    ...
    def __init__(self, thr_min, thr_max, blur=15, blob_params=None, detection_window=None):
        ...
        print ("<< Subscribed to topic /camera/depth/image_raw")
    ...
    def callback(self,data):
        ...
        if cols > 60 and rows > 60 :
            #--- Detect blobs
            keypoints, self.mask   = blob_detect(cv_image, self._threshold[0], self._threshold[1], self._blur,
            ...
            try:
                ...
                self.mask_pub.publish(self.bridge.cv2_to_imgmsg(self.mask, "8UC1"))
            except CvBridgeError as e:
                print(e)            
            ...
            for i, keyPoint in enumerate(keypoints):
                ...
                if hasattr(self, 'depth_image'):
                    print ("<< Depth Image Exists >>")
                    indices = np.transpose(np.argwhere(self.mask == 255))
                    # print(indices)
                    maskComplement = np.zeros_like(self.mask)
                    maskComplement[indices[0], indices[1]] = 0
                    indices = np.transpose(np.argwhere(self.mask == 0))
                    maskComplement[indices[0], indices[1]] = 1
                    # print(maskComplement[60])
                    depthMask = np.multiply(self.depth_image, maskComplement).astype(np.float64)
                    # print(depthMask[60])
                    z = np.sum(depthMask)/np.count_nonzero(depthMask)
                self.blob_point.x = x
                self.blob_point.y = y
                self.blob_point.z = z
                print ("<< blobPoint.x = " + str(x) + " blobPoint.y = " + str(y) + " blobPoint.z = " + str(z))
